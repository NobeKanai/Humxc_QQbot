...
class client {
  constructor(oicqConfig, botConfig) {
    ...
    this.plugin = plugin;
    this.run = run;
    this.page = new page("Main", this.run);
    //加载插件
    ...
    oicqClient.on("system.online", () => {
      ...
      if (this.page.run._event.has("online")) {
        this.page.run._event.get("online").forEach((element) => {
          element(e);
        });
      }
    });
    ...
    oicqClient.on("message", (e) => {
      //触发事件插件
      if (this.page.run._event.has("message")) {
        this.page.run._event.get("message").forEach((element) => {
          element(e);
        });
      }
      ...
      if (messageType(e) == "text")
        if (this.page.run.keyWord.has(e.raw_message))
          this.page.run.keyWord.get(e.raw_message).forEach((element) => {
            element(e);
          });
      ...
      if (messageType(e) == "COMMAND") {
        ...
        if (this.page.run.command.has(command[0]))
          this.page.run.command.get(command[0]).forEach((element) => {
            element(e, command);
          });
      }
    });
